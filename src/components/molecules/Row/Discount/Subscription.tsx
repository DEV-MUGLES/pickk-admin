...
import { Input, Typography, DatePicker, Button, Tooltip, Popconfirm, Icon, message } from 'antd';
...
export default function SubscriptionDiscountRow({
  ...
  const handleSubscribeDiscountPeriodChange = date => {
      handleChange({subscribeDiscountStartPeriod: moment(date[0]).format('YYYY-MM-DD')
  ...
  <Wrapper>
        <TitleRow>
          ...
          <DiscountRateInput size="small" value={subscribeDiscountData.subscribeDiscountRate}
          ...
          <Popconfirm
                ...
                okText="예"
                cancelText="아니오"
                icon={<Icon type="question-circle-o" style={{color: Colors.Primary}}/>}
            >
